...
const AnalyticsPage = () => {
	...
	const fetchAnalytics = async (startDate?: string, endDate?: string) => {
		...
		try {
			...
			let url = `${process.env.NEXT_PUBLIC_API_URL || "http://backend.madeinarnhemland.com.au"}/api/admin/sales/analytics`;
	...
	const fetchChartData = useCallback(async (startDate?: string, endDate?: string) => {
		...
		try {
			...
			console.log("📊 Fetching chart data:", `${process.env.NEXT_PUBLIC_API_URL || "http://backend.madeinarnhemland.com.au"}${endpoint}`);
